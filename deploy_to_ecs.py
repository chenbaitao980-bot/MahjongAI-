...
⚠ 手机需把 DNS 设为 ECS 公网 IP（8.136.32.137），热更请求才会落到 ECS。
⚠ watchdog 守护：脚本会同步到 ECS 但不自动 enable/start，由你按需手动启用。
...
前提:
    - 本地 ssh/scp 可免密连 root@8.136.32.137（已配好密钥）
...
# 额外文件：watchdog 守护（独立进程，不在主服务重启链上）
# (本地路径, 远程绝对路径, 部署后置操作)
EXTRA_FILES = [
    ("scripts/mahjong-mitm-watchdog.sh",
     "/usr/local/bin/mahjong-mitm-watchdog.sh",
     "chmod +x /usr/local/bin/mahjong-mitm-watchdog.sh"),
    ("scripts/mahjong-mitm-watchdog.service",
     "/etc/systemd/system/mahjong-mitm-watchdog.service",
...
def main():
    ...
    # 1.5 同步 watchdog（独立进程，不重启旧服务）
    for local, remote, post in EXTRA_FILES:
        run(["scp", local, f"{ECS_USER}@{ECS_IP}:{remote}"])
        run(["ssh", f"{ECS_USER}@{ECS_IP}", post])
        print(f"  [watchdog] {local} -> {remote}")
    ...
    print("\n[OK] 部署完成")
    print("提醒: 安全组需放行 TCP 443 + UDP 53；手机 DNS 设为 8.136.32.137")
    print("watchdog: 已同步到 ECS，但未 enable / start，请按需：")
    print("    ssh root@8.136.32.137 'systemctl enable --now mahjong-mitm-watchdog'")
