...
稳定版右侧策略建议区域必须优先保证硬算明细可读，不得因布局高度不足导致底部关键数据默认不可见。

#### Scenario: 策略建议区域向上扩张

- WHEN 稳定版面板在常见桌面窗口高度中显示
- THEN 右侧“策略建议”区域应占用右侧主要垂直空间
- AND 硬算明细底部的数据可信度、候选重排、风险/提醒等内容应在区域内完整可见或可清晰滚动查看

#### Scenario: 按轻重缓急和种类着色

- WHEN 右侧硬算明细渲染当前策略数据
- THEN 强提醒、财神风险、数据不足等高优先级信息应使用警示色
- AND 当前建议、推荐来源、模型状态等行动/推荐信息应使用强调色
- AND 基础状态、预测、向听、听牌、有效进张、数据可信度等不同种类应使用可区分的辅助色
- AND 着色只影响 UI 展示，不改变策略计算结果或 `snapshot()` 数据结构
