...
## Session 13: OpenWrt ipk 对齐 Windows MITM (110KB + python3-light 兼容 + 三层 DNS 劫持)

**Date**: 2026-06-27
**Task**: OpenWrt ipk 对齐 Windows MITM (110KB + python3-light 兼容 + 三层 DNS 劫持)
**Branch**: `master`

### Summary

把软路由 ipk 从 85MB 砍到 110KB(只装 NetConf.luac+project.manifest 不打整 APK)；__init__.py 注入 logging/idna shim+socket.getfqdn 兜底+urllib 取代 requests 让 python3-light 跑通；DNS 劫持三层: UDP/TCP 53 redirect、DoT 853/DoH 公共 IP/IPv6 :53 全 reject、resolve-gxb.sh 解析 6 个 gxb-* 真实 CDN IP 加 DNAT 兜底手机 DNS 缓存；init.d 用 setsid+独立 watchdog.sh 避免 BusyBox shell heredoc 卡死+TLS https 探活；ipk 用 GNU_FORMAT 避开 PAX 0x78 typeflag；真机验证手机连 WiFi 后 [mitm] 192.168.6.211 host=gxb-oss.imeete.com 触发热更资源下载；新 spec backend/openwrt-deployment.md + skill ops/openwrt-mitm-deploy + 3 条 memory
...
## Session 14: noconfig 账户权限管理 + 用户持久化

**Date**: 2026-06-28
**Task**: noconfig 账户权限管理 + 用户持久化
**Branch**: `master`

### Summary

新增 account_store.py 独立模块(账户CRUD+JSON持久化+SHA256密码哈希); app.py 改造(role感知cookie/admin标签页/子账户过滤/账户API); user_store.py 增加JSON持久化(用户重启不丢失); 部署至ECS验证通过
