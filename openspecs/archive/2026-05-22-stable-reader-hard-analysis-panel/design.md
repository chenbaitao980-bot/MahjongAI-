...
当前右侧“策略建议”区域位于状态区下方，内部包含摘要和硬算明细两个文本块。窗口高度有限时，硬算明细的后半段容易落到可视区域底部之外。

实施方式：

1. 在 `ui/stable_battle_panel.py` 中调整 `StableBattlePanel` 右侧布局比例，减少上方空白/固定高度，增加策略建议区域的 stretch 和最小可视高度。
2. 保留现有 `StableHardAnalysis` 数据来源，不新增策略字段，不改硬算逻辑。
3. 将硬算明细渲染改为带颜色的 HTML/Rich Text，或使用等价的 QTextEdit 富文本能力：
   - 当前建议、推荐来源、模型状态使用主强调色。
   - 强提醒、财神风险、数据可信度不足使用警示色。
   - 对方预测、进度预测使用辅助信息色。
   - 基础状态、向听、听牌、有效进张使用常规信息色。
4. 维持只读展示，不增加交互控件，不改左侧实时数据/事件流的数据来源。

回滚方式：恢复右侧布局 stretch/高度设置，并把硬算明细恢复为 `setPlainText()` 纯文本渲染。
