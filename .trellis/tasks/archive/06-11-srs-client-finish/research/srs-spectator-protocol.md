...
**结论（2026-06-12 当时）**：云端能过 SRS 认证(flag=0)，但被动抓包解不开大厅登录帧 → 误判为"native 密钥墙、active 不可行、只能 VPN"。

### ★★★ 2026-06-12 Frida 实测推翻上面的"native 密钥墙"——active 加密层其实是通的 ★★★

真机 Frida hook（`frida/hook_lobby_key.js` + `run_lobby_hook.py`，gadget listen 模式 attach `com.xm.zjgamecenter`，hook `AES_set_encrypt_key`/`Encryption::encrypt`）在一次真实登录里抓到全部加密前明文 + 真密钥：

1. **大厅登录帧 = SRS 会话密钥加密**（不是 native 专有 key）。时间戳关联：应用层 135 帧全用会话密钥 `bb411ffd…`(AES-128，本会话 HandshakeRsp 下发)；EncryptVer 用默认 key；抓到的默认 key `f362…` 与静态分析逐字节一致（证明 hook 抓的是真 key，反篡改没清零 AES_set_encrypt_key 这一层）。
2. **大厅登录明文 = 稳定设备/账号字段，无 server nonce**：`00000000 | 0c"020000000000"(identify) | 05"10160"(osver) | 13"com.xm.zjgamecenter" | 1b"android,BundleVersion:1.6.x"`；另有 `f4140100(channelid) 0c"LOLLAPALOOZA" 87 https://thirdwx.qlogo.cn/...(头像)`；PlayerConnect 明文 `0207c51b00..newpt1084306678 d7295c4e..(pwd=sessionid)`。唯一变化是帧头 4B askid/序列号（**客户端自生成，非服务器质询**）。
3. **关键推论**：PlayerConnect(msg5)就是用会话密钥加密、且我们 diag 实测**服务器认了(flag=0)**；大厅登录帧同一把会话密钥 → **云端既能发被接受的 PlayerConnect，就能发大厅登录帧**，只差明文内容（已抓，稳定可重建）。
4. 之前 pcap 里"session_key 解大厅帧=乱码"的真因：CFB 是**会话密钥 epoch 内连续流/或帧序状态**，单帧 fresh-from-IV 解中段帧必乱码；Frida 直接抓 encrypt 输入明文，绕过 CFB 状态问题。

**修正结论**：active 路**不被加密墙阻断**。大厅登录 = 会话密钥(云端自有) + 稳定明文(抓一次可重建)。

**剩余真未知（加密之外，按依赖）**：① 复现时云端要按正确帧序送(让服务器 CFB 状态对上)——需抓一份完整有序帧列 ② 登录后服务器是否告知当前 roomid(返回牌局机制) ③ 旁观记录是否含自己手牌 ④ 单连接墙(重连踢手机)——这是"手机玩+云端同时读"的根本障碍；"云端当唯一读取端"模型则不受限。

诊断/取证脚本：`scripts/diag_srs_{sample,live,watch}.py`、`capture_srs_sessionid.py`、`scripts/analyze_lobby_dump.py`；`frida/hook_lobby_key.js`、`frida/run_lobby_hook.py`。
