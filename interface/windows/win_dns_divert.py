"""win_dns_divert.py — WinDivert 网络层 DNS 劫持（突破游戏硬编码公共 DNS）。

从 remote/noconfig/hijack/dns_divert.py 收回到 Windows 托盘包；逻辑原样保留，
只换内核引用（mahjong_mitm 包）+ 去掉仓库根 sys.path 注入。
...
前提:
  ...
  - 与 setup_mitm（443 + DnsResponder）配合：win_dns_divert 负责"抢"硬编码 DNS 的查询，
...
from mahjong_mitm.setup_mitm import HIJACK_DOMAINS

logger = logging.getLogger("windows.win_dns_divert")
