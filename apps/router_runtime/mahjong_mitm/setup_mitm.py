...
# 包根目录（apps/router_runtime/）——assets/game_base.apk 与默认证书目录据此定位。
# 抽取自 remote/noconfig/hijack/，已去除对仓库根的依赖（原 _REPO_ROOT + sys.path 注入）。
_PKG_DIR = os.path.dirname(os.path.abspath(__file__))
_RUNTIME_ROOT = os.path.dirname(_PKG_DIR)  # apps/router_runtime/
...
def _selftest() -> None:
    ...
    assert patched_vm["version"] == "2.5.9.3051", patched_vm["version"]  # 1.0.0.51 → +(1,5,9,3000) = 2.5.9.3051
