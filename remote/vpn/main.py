"""
main.py — MahjongAI VPN Relay 入口

从同目录 config.yaml 读取配置（fallback: ../relay/config_vpn.yaml），
...
_LOGGER = logging.getLogger("remote.vpn")

# ─── 默认配置路径 ────────────────────────────────────────────────

_DEFAULT_CONFIG = os.path.join(_HERE, "config.yaml")
_FALLBACK_CONFIG = os.path.join(_RELAY_DIR, "config_vpn.yaml")
...
def resolve_config_path(override: str = "") -> str:
    ...
    2. 同目录 config.yaml
    3. ../relay/config_vpn.yaml（fallback）
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="MahjongAI VPN Relay — VPN 模式独立服务",
...
示例:
  ...
  python main.py --config my_cfg.yaml    # 自定义配置
    ...
    # 将命令行端口注入配置（命令行优先）
    port = args.port if args.port != 8001 else int(cfg.get("port", 8001))
    if args.port != 8001:
        port = args.port
    cfg["port"] = port

    _LOGGER.info("=" * 50)
    _LOGGER.info("MahjongAI VPN Relay")
