...
#### 修改 `stable/mapping.py`
- **根本原因**：游戏协议 instance 编码的牌型顺序为**条→万→筒→字**，而代码沿用 `ALL_TILE_IDS`（万→筒→条→字），导致所有牌解码偏移一个维度
- **Bug 表现**：面板显示"3万"但游戏画面为"三条"，面板显示"1筒"但画面为"一万"，财神显示"9筒"实为"9万"——13张手牌全部错误
- **修复**：新增 `_GAME_INSTANCE_TILE_IDS`（条→万→筒→字顺序），instance context 改用此表而非 `ALL_TILE_IDS`
- 视觉识别模块（HOG/SVM）有自己的训练模型，不受此修改影响

---

### 2026-05-15 — 语音播报开头截断修复（SSML静音垫片）

#### 修改 `utils/tts.py`
- **根本原因**：音频设备省电模式下首次输出前几十 ms 被硬件丢弃，导致"打两万"只听到"万"
- **修复1**：线程启动时先播 300ms 静音（`<silence msec="300"/>`）预热音频设备
- **修复2**：每次播报前加 80ms 静音垫片，防止设备短暂休眠后再次截断
- 改用 `SVSFlagsAsync | SVSFIsXML` + `WaitUntilDone(-1)` 组合，确保每句完整播完再取下一条
- 语速调整为 `-2`（比 `-1` 更慢，吐字更清晰）
...
#### 修改 `utils/tts.py`
- **根本原因**：pyttsx3 要求引擎必须在同一线程创建和调用；原实现用多个一次性线程重用全局 `_engine`，COM 事件无法派发，`runAndWait()` 静默退出
- **修复**：改为单个持久 daemon 线程（`_tts_worker`）持有引擎，主线程通过 `queue.Queue` 投递文本，TTS 线程循环消费队列
- 新增 `_ensure_thread()`：检查线程存活状态，死亡时自动重建
