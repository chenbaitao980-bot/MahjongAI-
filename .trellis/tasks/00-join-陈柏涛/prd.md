...
`陈柏涛` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/陈柏涛/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/陈柏涛/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee 陈柏涛` to
...
python ./.trellis/scripts/task.py archive 00-join-陈柏涛
```
